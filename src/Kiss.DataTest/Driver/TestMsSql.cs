...
namespace Kiss.DataTest.Driver
{
    ...
    public class TestMsSqlDriver
    {
        ...
        public void TestCompileQuery()
        {
            ...
            string expected = @"
SELECT * FROM ( 
SELECT DISTINCT TOP 105 [cbool], [cint], [cfloat], [cdatetime], [cnumeric] AS [a_cnumeric], [cstring] AS [a_cstring], COUNT([cint]) AS [avg_cint], COUNT([cint]) AS [count_cint], SUM([cint]) AS [sum_cint], MIN([cint]) AS [min_cint], MAX([cint]) AS [max_cint], cint - 1 AS [exp_cint]  , ROW_NUMBER() OVER (ORDER BY [cint] ASC, [cfloat] ASC, [cnumeric] DESC, [cstring] DESC, [cdatetime] ASC ) AS [_kiss_rownumber] 
FROM [ttable]
WHERE
[cbool] = @P_0
AND
...
";
        ...
        public void TestTableSchema()
        {
            ...
            Assert.True(pk.IsReadOnly == true, "pk.IsReadOnly");
            ...
            Assert.True(cbytes.DbType == DbType.Binary, "cbytes.DbType");
            //Assert.True(cbytes.RuntimeType == typeof(byte[]), "cbytes.RuntimeType");
        ...
        public void TestProcedureSchema()
        {
            ...
            Assert.True(p3.Direction == ParameterDirection.InputOutput, "x.Direction");
